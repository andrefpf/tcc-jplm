...
* @Last Modified time: 2019-08-14 18:31:51
*/
...
BoxParser::ParsedBox BoxParser::parse(uint32_t t_box_code, ManagedStream& managed_stream) {
  ...
  if (auto it = map.find(t_box_code); it != map.end()) {
    ...
    return std::move(it->second(managed_stream));
  }
